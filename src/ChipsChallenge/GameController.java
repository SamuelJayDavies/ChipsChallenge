    ...
    private void movePlayer() {
        ...
        if(nextMove == KeyCode.W || nextMove == KeyCode.A
            ...
            if(collisionHandler.isValidMove(nextPosition)) {
                ...
                finalPosition = collisionOccuredActor(finalPosition, originalPosition, collisionHandler.checkForActorCollision(finalPosition), currentPlayer.getType());
                ...
                if(monster.getType() == ActorType.PINKBALL && movingActors.contains(monster.getType())) {
                    ...
                    if(collisionHandler.isValidMove(ballNextMove)) {
                        ...
                        if(originalPosition == finalPosition) {
                            ...
                            TileType lastTileType = tileLayer.getTileAt(originalPosition[0], originalPosition[1]).getType();
                            resetButton(lastTileType, originalPosition);
                        }
                    } else {
    ...
    */
    ...
    private int[] moveMonster(int[] newPosition, int[] originalPosition, Actor monster) {
        if(collisionHandler.isValidMove(newPosition)) {
            int[] finalPosition = collisionOccurredTile(newPosition, originalPosition, collisionHandler.checkForTileCollision(newPosition), monster.getType());
            finalPosition = collisionOccuredActor(finalPosition, originalPosition, collisionHandler.checkForActorCollision(finalPosition), monster.getType());
            if (originalPosition != finalPosition) {
                return newPosition;
            }
            ...
            case BLOCK:
                if(originalActor == ActorType.PLAYER) {
                    ...
                    if(collisionHandler.isValidMove(blockNextMove)) {
                        ...
                        if(collisionHandler.checkForActorCollision(blockNextMove) != ActorType.NOACTOR && collisionHandler.checkForActorCollision(blockNextMove) != ActorType.PLAYER) {
                            return originalPosition;
                        }
